...
void UIContext::doLabel(const Rect & r, const char * text, int style,float red, float green, float blue, float alpha)
{
    Rect rt;
    int nbLines;
    Rect rect = placeRect(m_painter->getLabelRect(r, text, rt, nbLines));

    m_painter->drawLabel(rect, text, rt, nbLines, isHover(rect), style,red,green,blue,alpha);
}
...
bool UIContext::doProgressBar(const Rect & r, float min, float max, double* value, int style)
{
    float fVal = *value;
    return doProgressBar(r,min,max,&fVal,style);
}

bool UIContext::doProgressBar(const Rect & r, float min, float max, float * value, int style)
{
    ...
    Rect rs;
    Rect rc;
    Rect rect = placeRect(m_painter->getProgressBarRect( r, rs, f, rc ));
    ...
    m_painter->drawProgressBar(rect, rs, f,*value, rc, isHover(rect), style);
