...
namespace SceneGraph {
	...
	public:
		...
		void SetCameraPose( const Eigen::Matrix4d& dTwc )
		{
			m_dTwc = dTwc;
		}

		void SetRobotPose( const Eigen::Matrix4d& dTwr )
		{
			m_dTwc = dTwr * m_dTrc;
		}
		...
		Eigen::Matrix4d GetRobotPose() const
		{
			return m_dTwc * m_dTcr;
		}

		///////////////////////////////////////////////////////////////////////////////
		//        void SetPose( const Eigen::Vector6d& dPose )
		//        {
		//            m_dPose = GLCart2T( dPose(0), dPose(1), dPose(2), dPose(3), dPose(4), dPose(5) );
		//        }

		///////////////////////////////////////////////////////////////////////////////

		void SetIntrinsics( const Eigen::Matrix3d& dK, float fNear = 0.1,	float fFar = 1000 )
		{
			m_dK = dK;
			...
			m_fNear = fNear;
			m_fFar = fFar;
			m_dKgl = (Eigen::Matrix4d)pangolin::ProjectionMatrixRDF_BottomLeft( m_nSensorWidth, m_nSensorHeight, m_dK( 0, 0 ), m_dK( 1, 1 ), m_dK( 0, 2 ), m_dK( 1, 2 ), fNear, fFar );
		}
		...
		void Init(
				...
				const Eigen::Matrix4d dTwr, //< Input: initial camera pose
				...
				)
		{
			...
			m_dTcr = Eigen::Matrix4d::Identity( );
			m_dTcr.block < 3, 3 > (0, 0) = RDFvision.transpose( ) * RDFrobot;
			m_dTrc = Eigen::Matrix4d::Identity( );
			m_dTrc.block < 3, 3 > (0, 0) = RDFrobot.transpose( ) * RDFvision;

			SetIntrinsics(dK, fNear, fFar);
			SetRobotPose(dTwr);
		...
		void Begin( )
		{
			...
			////	            glGetDoublev( GL_PROJECTION_MATRIX, m_dM.data() );
			//
			//				const pangolin::OpenGlMatrix glK = pangolin::ProjectionMatrixRUB_BottomLeft(m_nSensorWidth, m_nSensorHeight, m_dK(0,0), m_dK(1,1), m_dK(0,2), m_dK(1,2), 0.1, 1000.0 );
			//				m_dM = (Eigen::Matrix<double,4,4>)glK;
			//				glK.Load();
			//            }
			//
...
//            }
	...
	private:
		...
		Eigen::Matrix4d m_dTwc; // Camera to world transform (also modelview matrix)

		Eigen::Matrix4d m_dTcr; // robot to camera transform
		Eigen::Matrix4d m_dTrc; // camera to robot transform
